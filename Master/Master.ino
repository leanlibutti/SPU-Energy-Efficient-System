...
#define THINGER_SERVER "192.168.0.202"
...
int baseIp = 210;          // IP a partir de la cual otorgar IPs a nodos, 192.168.4.4

// Estructura que representa un grupo en el maestro
typedef struct t_group {
  t_group()
    : isOn(false), isConnected(false), name(""), isSleeping(false), ignorePir(false){};
  String name;
...
struct SensorInfo {
  struct PirInfo {
    PirInfo()
      : isConnected(false), isOn(false), motionDetected(false), motionDetectedOnSlave(false),
  ...
  struct dhtInfo {
    dhtInfo()
      : isConnected(false), requestedFromPanel(false){};
...
bool startPairingMode = true;  // Flag que indica que se debe empezar a emparejar los nodos
...
// ThingerWifi thing("iii_lidi", "prueba_placa", "123456"); Comentado por Lean
...
// Metodo agregado por Lean
void createDefaultRelayGroups(){
  relayGroups[0].name = "g1";
  relayGroups[0].isOn= false;
  relayGroups[0].isConnected = true;
  relayGroups[0].motionDetectionTime = 10;
}

void setup() {
  Serial.begin(115200);
  EEPROM.begin(MEMORY_SIZE);
  Serial.println("Paso Init 1");
  udp.begin(DEFAULT_PORT);
  Serial.println("Paso Init 2");
  // for (int i = 0; i < 50; i++) Serial.println(); Comentado por Lean

  baseIp = getBaseIp();  //Lee la memoria EEPROM en busca de la siguiente IP libre
  Serial.println("Paso Init 3");

  encoder.onMove(encoderMove);  // Asigna los eventos del encoder
  encoder.onClick(encoderClick);
  encoder.onHold(encoderHold);
  Serial.println("Paso Init 4");
  initCalendar();
  Serial.println("Paso Init 5");
  createDefaultRelayGroups();
  initRelayGroups();
  Serial.println("Paso Init 6");
  initVisualMenu();
  Serial.println("Paso Init 7");
  initMasterSensors();
  Serial.println("Paso Init 8");
  initControlPanel();
  Serial.println("Paso Init 9");
  // initThinger(); Comentado por Lean
  initWifi();
  Serial.println("Paso Init 10");
  initRTC();
  Serial.println("Paso Init 11");
  initMenu();
  Serial.println("Paso Init 12");
...
void handlePacket() {
  ...
  if ((node = getNode(udp.remoteIP()))) {  // Si el nodo ya esta registrado
    ...
    Serial.printf(" New node handle packet: %s", udp.remoteIP().toString().c_str());
...
// Comentado por lean (toda la funcion)
// void initThinger() {
...
void sendNewIP(Node* node) {
  ...
  IPAddress newIp(192, 168, 0, baseIp);     // se le agrega el resto de la direccion
...
void sendSensorData() {
  ...
  if (ambientRead /*|| !ambientDataSent*/) {
    // thing.stream(thing["ambient_sensor"]); Comentado por Lean
    // thing.write_bucket("GeneralAmbientData", "ambient_sensor"); Comentado por Lean
    //if(ambientRead)
    // thing.call_endpoint("NodeRedAmbientData", thing["complete_ambient_sensor"]); Comentado por Lean
...
void initCalendar() {
  Serial.println("Paso Calendar 1");
  calendar = AlarmCalendar();
  Serial.println("Paso Calendar 2");
  ...
  Serial.println("Paso Calendar 3");
  calendarCheckAlarm = Alarm.timerRepeat(5, checkCalendarAlarms);
  Serial.println("Paso Calendar 4");
}
...
//   Serial.printf("\n Connecting relay 1");
//   if(args[0].equalsIgnoreCase("ALL")){
//     sendGlobalMessage(CONNECT_RELAY_MSG);
//   } else if(args[0] != ""){
//     int node = args[0].toInt();
//     if(node < connectedNodes){
...
void checkConnection() {
  if (WiFi.status() == WL_CONNECTED) {
    return;
    // thing.handle(); Comentado por Lean
...
void initWifi() {
  ...
  char externalSSID[MAX_SSID_LENGTH] = "Proyecto_Energia",
       externalPasswd[MAX_PASSWD_LENGTH] = "PrOyEcTo2023!",
       externalIp[16] = "192.168.0.199";
  ...
  if (1) {  //readExternalWifiInfo(externalSSID, externalPasswd, externalIp)){
    ...
    if (tries < 60) {
      Serial.printf("\n Connected to wifi.");
      // thing.handle();  // Fuerza la conexión a thinger, para poder enviar el estado del dht luego - Comentado por Lean
    ...
    while ((millis() - start) < seconds * 1000) {
      while (udp.parsePacket()) {
        ...
        if ((node = getNode(udp.remoteIP()))) {  // Si la longitud es mayor a 2 bytes se trata de un paquete
          ...
          printActualTime();  // Si es un nodo nuevo
          Serial.printf(" New node start pairing: %s", udp.remoteIP().toString().c_str());
          if (connectedNodes < MAX_NODES) {  // Se crea una instancia para el nodo
            nodes[connectedNodes] = new Node(udp, udp.remoteIP());
            nodes[connectedNodes]->sendAck();
            connectedNodes++;
...
void turnOnGroup(int group) {
  ...
  // thing.stream(thing["groups_state"]); Comentado por Lean
  // thing.stream(thing["actuator_info"]); Comentado por Lean
  // thing.write_bucket("ActuatorInfo", "actuator_info"); Comentado por Lean
  char bucket[50];
  snprintf(bucket, 50, "%sGroups", nodeName);
  // thing.write_bucket(bucket, "groups_state"); Comentado por Lean
  // thing.call_endpoint("NodeRedGroupState", thing["complete_groups_state"]); Comentado por Lean
}
...
void updateRelayGroups(int id) {
  ...
  for (int i = 2; i < GROUP_AMOUNT; i++) {
    if (relayGroups[i].isConnected) {
      ...
      LCDFunction* f1= new LCDFunction(title, deleteRelayGroup, i); 
      f1->makeDisposable();
      menuGrupos->addItem(f1);
      char title2[MAX_LINE_WIDTH];
      snprintf(title2, MAX_LINE_WIDTH, "Grupo %s", relayGroups[i].name.c_str());
      LCDItem* f2 = new LCDItem(title2);
      f2->makeDisposable();
      menuGrupos->addItem(f2);
    }
  }
